...
const { addVolunteersValidation, updateVolunteersValidation, deleteVolunteersValidation } = require('../../../validators/volunteers');
...
router.put('/:id', updateVolunteersValidation(), (req, res)=>{
  const errors = validationResult(req);
  if (!errors.isEmpty()) {
    return res.status(400).json({ errors: errors.array() });
  }

  volunteers_controller.update(req, res)
})
